...
export default async function Space() {
  ...
  return <>
    ...
    <HomeWidgets config={normalizeHomeWidgets(couple.homeWidgets)} latest={snapshot.latest} milestone={snapshot.milestone} memoryCount={snapshot.total} heartPhotoSrc={heart.photos?.[0] ? `/api/heart-photo/${heart.photos[0].id}` : undefined} />
  </>;
}
